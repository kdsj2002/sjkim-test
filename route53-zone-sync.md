...
apex(도메인 자체)의 SOA는 Route 53이 호스트존 만들 때 알아서 채우는 값이라
아예 비교 대상에서 뺀다. apex NS도 마찬가지로 기본은 건드리지 않는다.
Route 53이 배정한 네임서버를 zone 파일에 있는 옛날 네임서버로 덮어쓰면
위임이 깨지기 때문이다. 다만 차이가 있으면 화면에 정보로만 보여준다.

서브도메인 NS(위임용, 예: `sub.example.com NS ns1.other.com.`)는 이 예외에
안 걸리고 정상적으로 비교·적용된다. 정말 apex NS까지 맞추고 싶으면
`--allow-apex-ns`로 강제할 수 있다 (거의 쓸 일 없다).
